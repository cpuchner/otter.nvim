...
local function get_type_definition_sync(node, bufnr)
	...
	-- Get position information from the node
	local start_row, start_col, _, _ = node:range()

	-- Convert to 0-based indices that LSP uses
	local params = {
		textDocument = {
	...
	local timeout_ms = 1000 -- 1 second timeout
	local results = vim.lsp.buf_request_sync(bufnr, 'textDocument/typeDefinition', params, timeout_ms)
